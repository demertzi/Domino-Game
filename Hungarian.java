...
public class Hungarian {
    ...
    /*
    * Δημιουργεί τα hand των παικτών και τα κάνει shuffle
    */
    
    public Hungarian(Player player, Player bot, Table table){
        player1 = player;
        player2 = bot;
        this.table = table;
        
        player1.createHand(12);
        player2.createHand(12);
        player1.shuffleHand();
        player2.shuffleHand();
    }
    
    public Hungarian(Player player, Player bot1, Player bot2, Table table){
        player1 = player;
        player2 = bot1;
        player3 = bot2;
        this.table = table;
        
        player1.createHand(8);
        player2.createHand(8);
        player3.createHand(8);
        player1.shuffleHand();
        player2.shuffleHand();
        player3.shuffleHand();
    }
    
    public Hungarian(Player player, Player bot1, Player bot2, Player bot3, Table table){
        ...
        player1.createHand(6);
        player2.createHand(6);
        player3.createHand(6);
        player4.createHand(6);
        player1.shuffleHand();
        player2.shuffleHand();
        player3.shuffleHand();
        player4.shuffleHand();
    ...
    */
    ...
    public boolean putToSide(Tile t){
        ...
        if (table.isEmpty()){
            ...
            if (t.getSide1() == table.getRightSide()){
                shouldFlip = false;
                flag = false;
            }   
            else if (t.getSide2() == table.getRightSide()){
                shouldFlip = true;
                flag = false;
                }
            else if (t.getSide1() == table.getLeftSide()){
                    shouldFlip = true;
                    flag = true;
                    }
            else if (t.getSide2() == table.getLeftSide()){
                        shouldFlip = false;
                        flag = true;
                        }
                    }
    ...
    public void bot(Player player, boolean first){
        ...
        if (first){
          ...
          if (player.canPlay(table.getLeftSide())){
            t = player.tileFromHand(table.getLeftSide());
            putTileOnTable(t);
        }
        else{
            t = player.tileFromHand(table.getRightSide());
            putTileOnTable(t);
        }   
        }
        player.removeTile(t);
